...
public static class StaffMembersOnDutyReport
{
    public static byte[] Generate(List<StaffMemberModuleDto> staffMembers)
    {
        ...
        string fileName = "Staff Members On Duty Report";
        var title = fileName;
        ...
        worksheet.Cell(row, 2).Value = "Initials";
        ...
        {
            worksheet.Cell(row, 1).Value = staffMember.ModuleCode;
            worksheet.Cell(row, 2).Value = staffMember.Initials;
            worksheet.Cell(row, 3).Value = staffMember.LastName;
            worksheet.Cell(row, 3).Value = staffMember.Email;
            row++;
        }

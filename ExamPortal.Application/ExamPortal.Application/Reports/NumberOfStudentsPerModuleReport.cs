...
public static class NumberOfStudentsPerModuleReport
{
    public static byte[] Generate(List<ModuleCountDto> modules)
    {
        ...
        string fileName = "Number Of Students Per Module";
        var title = fileName;
        ...
        {
            worksheet.Cell(row, 1).Value = module.ModuleCode;
            worksheet.Cell(row, 2).Value = module.Description;
            worksheet.Cell(row, 3).Value = module.Count;
            row++;
        }
